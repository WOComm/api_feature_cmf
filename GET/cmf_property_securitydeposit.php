...
Flight::route('GET /cmf/property/securitydeposit/@id', function( $property_uid )
	{
	...
	if ( isset ($response->data->response)) {
		if (!empty($response->data->response)) {
			foreach ($response->data->response as $extra ) {
				if($extra->name == jr_gettext('_CMF_SECURITY_STRING','_CMF_SECURITY_STRING',false , false ) ) {
					$security_deposit = $extra->price;
				}
			}
		}
	}
	
	Flight::json( $response_name = "response" , $security_deposit ); 
	});
